...
video_stream_link = "http://192.168.1.113/video"  # Replace with your video stream link
...
while True:
    ...
    if a != -1 and b != -1:
        ...
        # Ensure jpg is not empty before decoding
        if len(jpg) > 0:
            ...
            if frame is not None:
                ...
                if (face_detected and not unknown_face_detected) and (time.time() - last_update_time >= update_interval):
                    ...
                    try:
                        response = requests.get(BLYNK_URL + "V6=1")  # Send 1 to V6 to indicate face recognition
                    ...
                    try:
                        response = requests.get(BLYNK_URL + "V6=0")  # Send 0 to V6 to indicate no face or unknown face
